...
class SimplePdf:
    ...
    def add_page(self, lines: list[tuple[str, int, str]], footer: str | None = None) -> None:
        ...
        for text, size, style in lines:
            font = "/F1" if style == "regular" else "/F2"
    ...
    def build_page_lines(self, title: str, body: list[str], page_number: int, total_pages: int) -> None:
        ...
        lines.append((f"{page_number}. {title}", 18, "bold"))
        lines.append(("", 8, "regular"))
        for idx, paragraph in enumerate(body, 1):
            wrapped = textwrap.wrap(paragraph, width=82)
            if paragraph.endswith(".") and not paragraph.startswith("GET"):
                prefix = f"{idx}. "
            else:
                prefix = f"{idx}. "
            for line_index, line in enumerate(wrapped):
                lines.append(((prefix if line_index == 0 else "   ") + line, 10, "regular"))
            lines.append(("", 5, "regular"))
        lines.append(("Key explanation note:", 12, "bold"))
        note = (
            "This page is intentionally concise so the PDF can be used as a speaking guide. "
            "During a live explanation, connect this topic back to app.js, the models folder, "
            "the EJS templates, or the public stylesheet depending on the page theme."
        )
        for line in textwrap.wrap(note, width=82):
            lines.append((line, 10, "regular"))
